...
import com.cometchat.chatuikit.shared.resources.utils.MediaUtils.getDataColumn
import com.google.gson.Gson
import com.hbisoft.hbrecorder.HBRecorder
import com.hbisoft.hbrecorder.HBRecorderListener
import com.telemechanic.consu.HomeActivity.Companion
...
class FlutterBridgeActivity : AppCompatActivity(),
    ...
    companion object {
        ...
        private val FILE_PICKER_REQUEST_CODE = 2
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        meetingId = intent.extras?.getString("ROOM_ID_KEY") as String
        name = intent.extras?.getString("LOGIN_NAME_KEY") as String
        myUid = intent.extras?.getString("LOGIN_UID") as String
        oppositeUid = intent.extras?.getString("REMOTE_UID") as String
        userType = intent.extras?.getString("ROLE_TYPE") as String
        meetingTimer= intent.extras?.getString("MEETING_TIMER") as String

        logger.info("FlutterBridge", "userType:${userType.toString()}")
        logger.info("FlutterBridge","name:${name.toString()}")
        logger.info("FlutterBridge","myUid:${myUid.toString()}")
        logger.info("FlutterBridge","oppositeUid:${oppositeUid.toString()}")
        logger.info("FlutterBridge","meetingId:${meetingId.toString()}")
        logger.info("FlutterBridge","meetingTimer:${meetingTimer.toString()}")
        ...
        chatMessageAdapter = ChatMessageAdapter(this, myUid!!,object :ChatMessageAdapter.ChatClicks{
                ...
                result.data?.data?.let { fileUri ->
                    ...
                    if (filePath != null) {
                        ...
                        if (cachedFile != null) {
                            ...
                            Toast.makeText(this, "Failed to retrieve file path", Toast.LENGTH_SHORT)
                                .show()
                        }
    ...
    private fun joinMeeting() {
        ...
        if (hasPermissionsAlready()) {
            authenticate(testUrl, meetingId, name)
        ...
        uiScope.launch {
            ...
            if (!meetingUrl.startsWith("http")) {
                ...
                if (meetingResponseJson == null) {
                    ...
                    meetingEndpointUrl =meetingUrl
                    mediaProjectionManager =
                        getSystemService(Context.MEDIA_PROJECTION_SERVICE) as MediaProjectionManager
                    powerManager = getSystemService(Context.POWER_SERVICE) as PowerManager
                    setupAudioVideoFacadeObservers()
                    val audioMode = audioVideoConfig.audioMode.value.let { intValue ->
                        AudioMode.from(intValue, defaultAudioMode = AudioMode.Stereo48K)
                    } ?: AudioMode.Stereo48K
                    val audioDeviceCapabilities =audioVideoConfig.audioDeviceCapabilities
                            ?: AudioDeviceCapabilities.InputAndOutput
                    val enableAudioRedundancy =audioVideoConfig.enableAudioRedundancy
                    val reconnectTimeoutMs = audioVideoConfig.reconnectTimeoutMs
                    audioVideoConfig = AudioVideoConfiguration(
                        audioMode = audioMode,
                        audioDeviceCapabilities = audioDeviceCapabilities,
...
//        }
    ...
    private fun getFileType(filePath: String): String {
        return when {
            filePath.endsWith(".jpg", true) || filePath.endsWith(".jpeg", true) || filePath.endsWith(".png", true) -> {
    ...
    private fun saveUriToTempFile(uri: Uri): File? {
        return try {
            ...
            Log.d("TempFile", "Failed to save URI to temp file: ${e.message}")
            null
        }
    }
    private fun sendMediaMessage(filePath: String, messageType: String) {
        ...
        CometChat.sendMediaMessage(mediaMessage, object : CometChat.CallbackListener<MediaMessage>() {
            override fun onSuccess(mediaMessage: MediaMessage) {
                ...
                var attachmentData=Attachment(mediaMessage.attachment.fileName,mediaMessage.attachment.fileExtension,mediaMessage.attachment.fileSize,mediaMessage.attachment.fileMimeType,mediaMessage.attachment.fileUrl)
            ...
            override fun onError(e: CometChatException) {
                ...
                Toast.makeText(this@FlutterBridgeActivity, "Failed to send media message: ${e.message}", Toast.LENGTH_SHORT).show()
            }
        })
    }
    ...
    private fun chatListeners()
    {
        CometChat.addMessageListener(listenerID, object : CometChat.MessageListener() {
            override fun onTextMessageReceived(textMessage: TextMessage) {
                ...
                if(isChatWindowOpen)
                {
                    CometChat.markAsRead(textMessage.id,textMessage.sender.uid,CometChatConstants.RECEIVER_TYPE_USER,textMessage.sender.uid)
                }
            }
            ...
            override fun onMediaMessageReceived(mediaMessage: MediaMessage) {
                ...
                if(isChatWindowOpen)
                {
                    CometChat.markAsRead(mediaMessage.id,mediaMessage.sender.uid,CometChatConstants.RECEIVER_TYPE_USER,mediaMessage.sender.uid)
                }
            }
...
//            oppositeUid.toString(),
//            object : CometChat.CallbackListener<HashMap<String?, Int?>?>() {
//
//                override fun onSuccess(stringIntegerHashMap: HashMap<String?, Int?>?) {
//                    Log.d("UnreadMessageCount", stringIntegerHashMap?.getValue("UID").toString())
//                }
//
//                override fun onError(e: CometChatException) {
//                    Log.e("UnreadMessageCountError", "Failed to fetch unread message count: ${e.message}")
//                }
//            }
//        )
    }
    ...
    fun showBottomSheet() {
        ...
        binding.apply {
            ...
            // Set up RecyclerView
            recyMessage.layoutManager = LinearLayoutManager(this@FlutterBridgeActivity)
            ...
            llChatSend.setOnClickListener {
                if (etMsg.text.toString().isEmpty()) {
                    Toast.makeText(
                        this@FlutterBridgeActivity,
                        "Please Type Something... ",
                        Toast.LENGTH_SHORT
                    ).show()
                } else {
                    var receiverId = oppositeUid.toString()
...
//            }
    ...
    fun receiveCoordsData(coordinates: ArrayList<String>) {
        ...
        if (remoteIsSmall && userType != "consumer") {
        ...
        } else if (!remoteIsSmall && userType != "consumer") {
//            val bigViewWidth = binding.bigVideoSurface.width.toFloat()
//            val bigViewHeight = binding.bigVideoSurface.height.toFloat()
...
//        }
    ...
    private fun showVideoTile(tileState: VideoTileState) {
        ...
        if (tileState.isLocalTile) {
            ...
            Log.d("getShowVideoTile:","working")
...
//                binding.blurViewBottom.scaleX = 1f
    ...
    override fun onResume() {
        ...
        if (!meetingModel.isLocalVideoStarted) {
            ...
            Log.d("getResumeStatus","here1")
        }

        val updatedSources = meetingModel.getRemoteVideoSourceConfigurations()
        audioVideo.updateVideoSourceSubscriptions(updatedSources, emptyArray())
            Log.d("getResumeStatus","here2")
            if (meetingModel.getRemoteVideoTileStates().isNotEmpty()) {
                Log.d("getResumeStatus", "coming here")
                meetingModel.getRemoteVideoTileStates()[0].let {
                    if (it.videoTileState.pauseState.equals(true)) {
                        audioVideo.resumeRemoteVideoTile(it.videoTileState.tileId)
                        updateSingleVideoView()
                        Log.d("getResumeStatus", it.attendeeName)
                    }
                }
            }
    ...
    private fun startCountdownTimer(timeInMillis: Long) {
        countdownTimer = object : CountDownTimer(timeInMillis, 1000) {
            override fun onTick(millisUntilFinished: Long) {
                ...
                if (timeFormatted == "01:00" && !isInExtraTime && userType == "consumer") {
...
startCountdownTimer(meetingTimer.toLong() * 60 * 1000)
                }
                Log.d("2Devices", "true")
                if (isPortrait) {
                    audioVideo.realtimeSendDataMessage("portrait", "", 5000)
