...
import com.tele.android.databinding.ActivityHomeBinding
...
class HomeActivity : AppCompatActivity(),
    ...
    private fun moveCursor(x: Float, y: Float) {
        ...
        binding.bigVideoSurface.getLocationOnScreen(location)
        val offsetX = location[0].toFloat()
        val offsetY = location[1].toFloat()

// Step 2: Calculate visible width and height of `bigVideoSurface`
        val bigViewWidth = binding.bigVideoSurface.width.toFloat()
        val bigViewHeight = binding.bigVideoSurface.height.toFloat()

// Step 3: Calculate cursor half dimensions for center positioning
        val cursorHalfWidth = binding.cursor.width / 2
        val cursorHalfHeight = binding.cursor.height / 2

// Step 4: Define boundaries to keep the cursor within `bigVideoSurface` bounds
        val minX = offsetX + cursorHalfWidth
        val maxX = offsetX + bigViewWidth - cursorHalfWidth
        val minY = offsetY + cursorHalfHeight
        val maxY = offsetY + bigViewHeight - cursorHalfHeight

// Step 5: Adjusted coordinates within `bigVideoSurface` bounds
        val clampedX = (x + offsetX).coerceIn(minX, maxX)
        val clampedY = (y + offsetY).coerceIn(minY, maxY)

// Step 6: Place cursor at the clamped position within `bigVideoSurface` bounds
        binding.cursor.x = clampedX - offsetX - cursorHalfWidth
        binding.cursor.y = clampedY - offsetY - cursorHalfHeight

// Update moving coordinates if needed for further logic
        movingX = clampedX - offsetX
        movingY = clampedY - offsetY
        toSendCoordsData()
    }
...
//                }
    ...
    fun receiveCoordsData(coordinates: ArrayList<String>) {
        ...
        if (remoteIsSmall && name != "user") {
            val location = IntArray(2)
            binding.videoSmall.getLocationInSurface(location)
            val offsetX = location[0].toFloat()
            val offsetY = location[1].toFloat()
            smallViewWidth = binding.videoSmall.width.toFloat()
            smallViewHeight = binding.videoSmall.height.toFloat()
            movingSmallX = relativeOppositeX * smallViewWidth+offsetX
            movingSmallY = relativeOppositeY * smallViewHeight+offsetY
            ...
            val location = IntArray(2)
            binding.bigVideoSurface.getLocationInSurface(location)
            val offsetX = location[0].toFloat()
            val offsetY = location[1].toFloat()
            bigViewWidth = binding.bigVideoSurface.width.toFloat()
            bigViewHeight = binding.bigVideoSurface.height.toFloat()
...
// mainViewHeight:2131.0,mainViewWidth:1080.0
    ...
    val mainViewWidth = binding.bigVideoSurface.width.toFloat()
        ...
        Log.d("HomeCase1","smallContainerHeight:"+smallContainerHeight+",smallContainerWidth:"+smallContainerWidth)
...
//    mainViewHeight:1473.0,mainViewWidth:720.0
     //   mainViewHeight:405.0,mainViewWidth:720.0
        if (isRemoteVideoMaximized) {
            ...
            if (isPortrait && isRemotePortrait || !isPortrait && !isRemotePortrait) {
                ...
                largeViewParams.width = (mainViewWidth * 9 / 16).toInt()
            }
